...
use app\enterprise\model\Adv as AdvModel;
...
class  Adv extends Admin
{
    ...
    //获取数据
		...
		$map = $this->getMap();
        //获取数据
        $dataList = AdvModel::where($order)->where($map)->order('id desc')->paginate();
		// 分页数据
		$page = $dataList->render();
        //获取当前所在
        $datamodelID = ModelModel::where(array('table' => 'cj_enterprise_adv','status'=>1))->value('id');
     ...
     */
     public function add(){
       if($this->request->isPost()){
             ...
             //判断数据是否重复添加
             $datappp =AdvModel::where($datas)->find();
             if($datappp){
                 $this->error('数据重复');
             }
             $dataadd = AdvModel::create($datas);
             if($dataadd){
     ...
     public function edit($id=''){
		if($this->request->isPost()){
			...
			// 验证
			//$result = $this->validate($data, 'Adv.edit');
