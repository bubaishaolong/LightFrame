...
class  Member extends Admin
{
    ...
    //获取数据
		...
		$map = $this->getMap();
        //获取数据
        $dataList = MemberModel::where($order)->where($map)->order('id desc')->paginate();
		// 分页数据
		$page = $dataList->render();
        //获取当前所在
        $datamodelID = ModelModel::where(array('table' => 'cj_shop_member','status'=>1))->value('id');
     ...
     */
     public function add(){
       if($this->request->isPost()){
             ...
             //判断数据是否重复添加
             $datappp =MemberModel::where($datas)->find();
             if($datappp){
                 $this->error('数据重复');
             }
             $dataadd = MemberModel::create($datas);
             if($dataadd){
