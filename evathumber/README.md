...
修改config.local.php(没有就复制config.default.php)
  'source_path' => __DIR__ . '/../public/uploads/images',//主图所在文件
  'thumb_cache_path' => __DIR__ . '/../public/uploads/thumb',//缩略图的生成目录   
//获取图片的名字  path 是文件的完整路径
        $img = pathinfo('http://127.0.0.1/public/uploads/images/cf.gif');
        //生成缩略图
        $imgs= 'http://'.$_SERVER['SERVER_NAME'].'/EvaThumber/thumb/d/'.$img['filename'];//这里是文件处理的路径
        $this->assign('img',$imgs);
        //前端代码调用
        <div class="span2"><img src="{$img},w_160.png"></div>